...
export class ResultService {
  ...
  console.log('ResultService - Starting to get comptage results');
  ...
  return forkJoin({
    ...
    map(result => {
      ...
      console.log('ResultService - Received comptages:', comptages.length);
      console.log('ResultService - Received operators:', operators.length);
      ...
      console.log('ResultService - Created operators map with', operatorsMap.size, 'entries');
      
      // Grouper les comptages par référence (ref$lot$sous)
      const comptageGroups = new Map<string, Comptage[]>();
      let skippedComptages = 0;
      ...
      comptages.forEach(comptage => {
        ...
        if (parts.length < 4) {
          skippedComptages++;
          console.log('ResultService - Skipping comptage with invalid reference format:', comptage.reference);
          return;
        }
      ...
      console.log('ResultService - Grouped comptages into', comptageGroups.size, 'groups');
      console.log('ResultService - Skipped', skippedComptages, 'comptages due to invalid reference format');
      
      // Log a sample of the first group if available
      if (comptageGroups.size > 0) {
        const firstKey = Array.from(comptageGroups.keys())[0];
        console.log('ResultService - Sample group key:', firstKey);
        console.log('ResultService - Sample group comptages:', comptageGroups.get(firstKey)?.length);
      }
      ...
      console.log('ResultService - Created', results.length, 'comptage results');
    ...
    catchError(error => {
      console.error('ResultService - ERROR retrieving results:', error);
      return of([]);
    })
  );
}
